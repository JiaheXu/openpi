...
class ImageRecorder:
    def __init__(self, node, is_debug=False):
        ...
        for idx, cam_name in enumerate(self.camera_names):
            ...
            self.node.create_subscription( Image, f"/{self.topic_names[idx]}", callback_func, 1)
...
class Recorder:
    def __init__(self, side, node, is_debug=False):
